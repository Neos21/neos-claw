...
export class McpClient {
  ...
  /** 1 サーバを起動し、ツールを発見して AgentCore に登録する */
  private async startServer(
    ...
    const client = new Client({ name: `neos-claw-${name}`, version: '1.0.0' });
    ...
    this.log(`  Tools From "${name}" : ${tools.map((tool: any) => tool.name).join(', ')}`);
    ...
    for(const tool of tools) {
      const agentTool: AgentTool = {
        ...
        // クロージャで `client` と `tool.name` をキャプチャする
        execute: async args => {
