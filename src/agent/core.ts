...
export const confirmPrefix = '__CONFIRM__:';
...
export class AgentCore {
   ...
   */
  public async run(history: Array<Message>, userMessage: string): Promise<RunResult> {
    ...
    this.log('▶ Starting ReAct Loop', { model: this.model, tools: this.tools.map(tool => tool.name) });
    ...
    while(iterations < this.maxIterations) {
      ...
      for(const toolCall of assistantMessage.tool_calls as Array<ToolCall>) {
        ...
        // ToolProxy が確認待ちを返した場合はループを打ち切り確認メッセージをそのまま最終回答として返す
        if(result.startsWith(confirmPrefix)) {
          confirmText = result.slice(confirmPrefix.length);
          break;
        }
