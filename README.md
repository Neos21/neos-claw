...
1. Brave Search で note・Zenn の類似記事を検索
...
- **WebSocket** : ストリーミング応答対応 ()`ws` or `socket.io`)
- **Web UI** : シンプルなチャット画面 (HTML + Vanilla JS or React)
...
| Brave Search | Web検索 (月2,000クエリ無料)     | `@modelcontextprotocol/server-brave-search` |
...
| 分類             | 技術                        | 用途                          |
|------------------|-----------------------------|-------------------------------|
| ランタイム       | Node.js v22 以降 + tsx      | TypeScript を直接実行         |
| 言語             | TypeScript                  | 全コンポーネント共通          |
| LLM              | Ollama + qwen2.5:14b-Q4_K_M | ローカル推論                  |
| ツール管理       | MCP (stdio)                 | ファイル・Web・カスタムツール |
| MCP SDK          | `@modelcontextprotocol/sdk` | MCP クライアント実装          |
| Ollama SDK       | `ollama` (npm)              | Ollama API クライアント       |
| Web サーバ       | Express                     | REST API + 静的ファイル配信   |
| リアルタイム通信 | ws                          | WebSocket ストリーミング      |
| Slack Bot        | `@slack/bolt`               | Slack アダプタ                |
| Discord Bot      | `discord.js`                | Discord アダプタ              |
| 設定管理         | `dotenv`                    | 環境変数                      |
...
# Slack (不要なら SLACK_ENABLED=false のまま)
SLACK_ENABLED=true
SLACK_BOT_TOKEN=xoxb-...
SLACK_SIGNING_SECRET=...
SLACK_APP_TOKEN=xapp-...  # Socket Mode 用

# Discord (不要なら DISCORD_ENABLED=false のまま)
...
- [ ] note・Zenn 記事ネタ判定エージェント (Brave Search + Fetch MCP)
